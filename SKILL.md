...
- `scripts/get_skill_module.sh` / `scripts/get_skill_module.ps1`：获取指定 Skill 模块详情
- `scripts/discovery_recommendations.sh` / `scripts/discovery_recommendations.ps1`：获取发现页推荐课程（需鉴权）
- `scripts/discovery_recent_courses.sh` / `scripts/discovery_recent_courses.ps1`：获取最近上线课程（需鉴权）
- `scripts/search_courses.sh` / `scripts/search_courses.ps1`：关键字搜课
- `scripts/get_trees.sh` / `scripts/get_trees.ps1`：获取课程体系树（需鉴权）
- `scripts/get_courses_by_tree.sh` / `scripts/get_courses_by_tree.ps1`：按体系查课程（需鉴权）
