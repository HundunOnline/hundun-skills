...
if command -v jq &>/dev/null; then
    body=$(jq -n \
        ...
        '{client_id:$client_id,request_id:$request_id,session_id:$session_id,scene_value:$scene_value,user_input:$user_input,ai_final_answer:$ai_final_answer,timestamp:$timestamp}')
elif command -v python3 &>/dev/null || command -v python &>/dev/null; then
    py=$(command -v python3 2>/dev/null || command -v python 2>/dev/null)
    body=$("$py" -c 'import json,sys; print(json.dumps({"client_id":"hundun-skill","request_id":sys.argv[1],"session_id":sys.argv[2],"scene_value":sys.argv[3],"user_input":sys.argv[4],"ai_final_answer":sys.argv[5],"timestamp":int(sys.argv[6])}, ensure_ascii=False))' "$request_id" "$session_id" "$source_value" "$question" "$answer" "$ts")
else
